...
def code_keyboard(code: str = "") -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton(str(n), callback_data=f"code_digit_{n}") for n in (1, 2, 3)],
        [InlineKeyboardButton(str(n), callback_data=f"code_digit_{n}") for n in (4, 5, 6)],
        [InlineKeyboardButton(str(n), callback_data=f"code_digit_{n}") for n in (7, 8, 9)],
        [
...
def code_display_text(code: str) -> str:
    dots = "".join("• " for _ in code) or "—"
    return f"Введите код из Telegram кнопками ниже:\n\n{dots}"
    ...
    if query.data == "list_groups":
        ...
        try:
            ...
            await query.message.reply_text(
                ...
                "Мониторинг остановлен. Если у вас была задана переменная окружения "
                "SESSION_STRING в Render — удалите или очистите её, иначе при следующем "
                "запуске сервис попробует использовать уже недействительную сессию.\n\n"
                "Чтобы войти заново — «🔑 Войти в аккаунт».",
                reply_markup=main_menu_keyboard(),
            )
        except Exception as e:
            await query.message.reply_text(f"Ошибка при выходе: {e}")
...
async def login_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(code_display_text(""), reply_markup=code_keyboard())
    return CODE
...
async def code_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("code_digit_"):
        ...
        await query.edit_message_text(code_display_text(code), reply_markup=code_keyboard(code))
        return CODE

    if data == "code_backspace":
        code = code[:-1]
        context.user_data["code"] = code
        await query.edit_message_text(code_display_text(code), reply_markup=code_keyboard(code))
        return CODE
    ...
    if data == "code_submit":
        ...
        try:
            ...
            await query.edit_message_text(
                ...
                "Введите пароль 2FA обычным текстовым сообщением:"
            )
            return PASSWORD
        except (PhoneCodeInvalidError, PhoneCodeExpiredError):
            context.user_data["code"] = ""
            await query.edit_message_text("Код неверный или устарел. Начните заново: /start")
            return ConversationHandler.END
        except Exception as e:
            await query.edit_message_text(f"Ошибка входа: {e}")
            return ConversationHandler.END

        await query.edit_message_text("Вход выполнен ✅")
        await _send_login_success(query.message)
        return ConversationHandler.END

    return CODE


# Резервный вариант: если пользователь всё же напишет код текстом
async def login_code_text_fallback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    code = update.message.text.strip().replace(" ", "")
    try:
        await userbot_manager.submit_code(code)
    except SessionPasswordNeededError:
        await update.message.reply_text("Введите пароль двухфакторной аутентификации (2FA):")
        return PASSWORD
    except (PhoneCodeInvalidError, PhoneCodeExpiredError):
    ...
    await _send_login_success(update.message)
    return ConversationHandler.END
...
async def _send_login_success(message):
...
def build_application() -> Application:
    ...
    login_conv = ConversationHandler(
        ...
        states={
            ...
            CODE: [
                CallbackQueryHandler(code_button, pattern="^code_"),
                MessageHandler(filters.TEXT & ~filters.COMMAND, login_code_text_fallback),
            ],
    ...
    application.add_handler(
        CallbackQueryHandler(
            simple_menu_handler,
            pattern="^(list_groups|start_monitor|export_db|stats|logout)$",
        )
    )
